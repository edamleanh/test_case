...
test('User can log in with valid credentials', async t => {
    ...
    const successMessage = Selector('h2').withText('Login');
    const loginToParticipateButton = Selector('input[type="submit"]').withAttribute('value', 'Log in to participate');
    ...
    await t
        ...
        .click(contestLink)
        .expect(loginToParticipateButton.exists).ok('Log in to participate button should be visible')
        .click(loginToParticipateButton)
